...
public class ACItemAdditionalRegistry {
    ...
    private static final List<LootPool> pool = new ArrayList<>();
    ...
    public static void addLoots(String poolName, int minValue, int maxValue, ACLootItem... entries) {
        ...
        pool.add(new LootPool(_entries.toArray(entries1), DEFAULT_CONDS, new RandomValueRange(minValue, maxValue), new RandomValueRange(0, 0), "academy:" + poolName));
    }

    @SubscribeEvent
    public void lootLoad(LootTableLoadEvent evt) {
        Arrays.stream(DEFAULT_APPEARANCE).filter(loc -> evt.getName().equals(loc)).forEach(loc -> pool.forEach(lootPool -> evt.getTable().addPool(lootPool)));
    }
}
